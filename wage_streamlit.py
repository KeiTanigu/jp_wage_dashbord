...
#ある列を基準として2つのデータをマージする場合はmergeを使う
#正規化処理　最小値0,最大値1とする処理のこと　Y=X-Xmin/Xmax-Xmin
...
#opacity:不透明度、threshold:どこの辺りをしきい値として設定するか、get_weigh:どこの値をヒートマップにするかを設定する
...
fig = px.scatter(df_mean_bubble,x="一人当たり賃金（万円）",y="年間賞与その他特別給与額（万円）",range_x=[150,700],range_y=[0,150],size="所定内給与額（万円）",size_max=38,color="年齢",animation_frame="集計年",animation_group="年齢")
st.plotly_chart(fig)
...
fig = px.bar(df_mean_categ,x=option_wage,y="産業大分類名",color="産業大分類名",animation_frame="年齢",range_x=[0,max_x],orientation="h",width=800,height=500)
st.plotly_chart(fig)
